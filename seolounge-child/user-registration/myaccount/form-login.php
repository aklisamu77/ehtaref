...
<div class="ur-frontend-form login <?php echo $template_class; ?>" id="ur-frontend-form">
	...
	<?php if (isset($_GET['success_regist'])) { ?> 
	<div class="ur-message user-registration-message" id="ur-submit-message-node">
			<ul class="">User successfully registered.</ul>
	</div>
	<?php } ?>
	<!-- -->
		...
		<div class="ur-form-row">
			<div class="ur-form-grid">
					...
					<p class="user-registration-form-row user-registration-form-row--wide form-row form-row-wide">
						...
						id="username" value="<?php echo ( ! empty( $_POST['username'] ) ) ? esc_attr( $_POST['username'] ) : ''; ?>"
						style=" border-radius: 4px; border: 1px #e0e0e0 solid; min-height: 45px; padding: 4px 8px; "
						/>
					...
					<p class="user-registration-form-row user-registration-form-row--wide form-row form-row-wide<?php echo ( 'yes' === get_option( 'user_registration_login_option_hide_show_password', 'no' ) ) ? ' hide_show_password' : ''; ?>">
						...
						<input style=" border-radius: 4px;" placeholder="<?php echo esc_attr( $placeholders['password'] ); ?>" class="user-registration-Input user-registration-Input--text input-text" type="password" name="password" id="password" />
					...
					<p class="form-row">
						...
						<div>
							...
							<span></span>
							<?php } else { ?>
							<input type="submit" style=" float: unset; border-radius: 30px; " class="user-registration-Button button " name="login" value="<?php echo esc_html( $labels['login'] ); ?>" />
							<?php } ?>
						</div>
